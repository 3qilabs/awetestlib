module Awetestlib
  ...
  class HtmlReport
    ...
    def add_to_report(message, location, result, level = 1)
      ...
      row = '<tr>
        <td class=' + left_class + ' width=' + @col_1_p + '><p class=' + pgph_class + '>' + msg + '</p></td>
  ...
  module Logging
    ...
    def mark_test_level(message = '', lvl = nil, desc = '', caller = 1, wai_lvl = 4, dbg = nil)
      ...
      @report_class.add_to_report(strg, '&nbsp', '&nbsp', lvl || 1) if @report_class
  ...
  module Regression
    module Browser
      ...
      def set_mobile_capabilities(device_id, device_type, sdk, http_client, server_url = "http://127.0.0.1:4723/wd/hub")
        mark_test_level
        ...
        case device_type
          ...
          when /android device/i
            desired_caps = {
                ...
                'chromeOptions'             => { "args" => '--verbose' } #ignore-certificate-errors=true homepage=about:blank test_type=true' }
            }
      ...
      def open_mobile_browser
        ...
        client.timeout = 300
        desired_caps   = set_mobile_capabilities(self.device_id, self.device_type, self.sdk, client)
      ...
      def close_browser(browser, where = @myName, lnbr = __LINE__)
        ...
        if $mobile
          ...
          sleep(1)
          if $emulator
            end_android_processes
            clean_up_android_temp
          end
    ...
    module Utilities
      ...
      def get_process_list
        ...
        if list =~ /No tasks are running which match the specified criteria/i
          ...
          tasks = csv.to_a.map { |row| row.to_hash }
          # debug_to_log(tasks)
        end
        tasks
      rescue
